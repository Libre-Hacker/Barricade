...
onready var navigation = find_parent("Navigation")
...
func calculate_path(targetPosition):
	pathPoints = navigation.get_simple_path(global_transform.origin, targetPosition)
	pathIndex = 0
...
func get_path_direction():
	...
	if(calculate_direction().length() < 0.3):
		pathIndex += 1
